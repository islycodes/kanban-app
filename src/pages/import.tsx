    ...
    reader.onload = async (event) => {
      try {
        ...
        const data = Array.isArray(parsedData)
          ? parsedData
          : ([parsedData] as DashboardInterface[]);
        for (const dashboard of data) {
          const { tickets, ...dashboardWithoutTickets } = dashboard;
          if (userId != null) {
            dashboardWithoutTickets.user_id = userId!;
          }
          const successData = await ApiInstance.addDashboardFunction({
            name: dashboardWithoutTickets.name,
            user_id: dashboardWithoutTickets.user_id,
