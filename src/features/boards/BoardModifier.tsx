...
import { IBoardData, IColumnPostBody, IColumn, IColumnEntities } from "../../@types/types";
...
export default function BoardModifier({ titles, selectedBoard, type }: { titles: string[]; selectedBoard: IBoardData | null; type: "add" | "edit" }) {
  const [modalTitle, nameTitle, saveText] = titles;
  ...
  const formatNewCol = useCallback(
    (name: string, id: string): IColumn => {
      if (!newColumns) throw new Error("newColumns must be defined to create a new column");
      return {
        name,
        id,
        board: "",
  ...
  function handleSaveBoard() {
    ...
    if (selectedBoard.id === "newBoard" || selectedBoard.name !== boardName) postBody.name = boardName;
    ...
    Object.values(newColumns).forEach((col) => {
      ...
      switch (operation) {
        ...
        case undefined:
          break;
        default:
          throw new Error(`Column has an invalid operation type: ${col.operation}`);
      }
    });
  ...
  const mappedColumnInputs = (
    <>
      ...
      {newColumns ? (
        Object.values(newColumns)?.map((column, index) => (column ? <ColumnInput column={column} setNewColumns={setNewColumns} key={`column-input-${index}`} /> : <></>))
      ) : (
        <></>
      )}
    </>
  ...
  return (
    <div className="px-2">
      ...
      <label className="mb-2 text-xs font-medium text-gray-500 dark:text-gray-300">{nameTitle}</label>
      <input
        data-testid="board_name_input"
        className={classNames("w-full text-sm dark:bg-primary-gray-700", "dark:border-primary-gray-600 dark:text-gray-300", "mb-6 py-2 px-3 border rounded")}
        value={boardName}
        type="text"
        onChange={(e) => {
      ...
      />
      <button onClick={handleSaveBoard} className="w-full py-2 bg-primary-indigo-active text-white rounded-full text-xs my-2">
        {saveText}
      </button>
    </div>
  );
